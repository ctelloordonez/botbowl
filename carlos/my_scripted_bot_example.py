...
def path_to_move_actions(game: botbowl.Game, path: Path, do_assertions=True) -> List[Action]:
    """
...
from carlos.my_proc_bot import get_data_path
import os


botbowl.register_bot('copied_scripted', MyCopiedScriptedBot)
if __name__ == "__main__":
    ...
    # Play 10 games
    for i in range(400):
        home_agent = botbowl.make_bot('copied_scripted')
        home_agent.name = "Home scripted"
        away_agent = botbowl.make_bot('copied_scripted')
        away_agent.name = "Away scripted"
        ...
        pairs = len(os.listdir(directory))
        print("\n Pairs count: ", (len(os.listdir(directory))))
